...
- (void)strokeChart
{
    ...
    if (_showLabel) {
        ...
        for (int index = 0; index < _xLabels.count; index++) {
            ...
            if (labelAddCount == _xLabelSkip) {
                ...
                [label sizeToFit];
                CGFloat labelXPosition  = (index *  _xLabelWidth + _chartMargin + _xLabelWidth /2.0 ) + 15;
    ...
    for (NSNumber *valueString in _yValues) {
        ...
        if (_barWidth) {
            barWidth = _barWidth;
            barXPosition = index *  _xLabelWidth + _chartMargin + _xLabelWidth /2.0 - _barWidth /2.0 + 15;
        }else{
            barXPosition = index *  _xLabelWidth + _chartMargin + _xLabelWidth * 0.25 + 15;
            if (_showLabel) {
